...
NERDCTL_VERSION="2.0.4" # Check https://github.com/containerd/nerdctl/releases for updates
archType="amd64"
...
if dpkg-query -W | grep amazon-ssm-agent; then
  echo "amazon-ssm-agent already present - skipping install"
else
  if snap list | grep -q amazon-ssm-agent; then
    echo "Removing conflicting Snap version of amazon-ssm-agent"
    sudo snap remove --purge amazon-ssm-agent
  fi

  if ! [[ -z "${SSM_AGENT_VERSION}" ]]; then
    echo "Downloading amazon-ssm-agent@${SSM_AGENT_VERSION} from S3"
    SSM_DEB_URL="https://s3.${BINARY_BUCKET_REGION}.${S3_DOMAIN}/amazon-ssm-${BINARY_BUCKET_REGION}/${SSM_AGENT_VERSION}/debian_${ARCH}/amazon-ssm-agent.deb"
    wget -q "${SSM_DEB_URL}" -O /tmp/amazon-ssm-agent.deb
  else
    echo "Installing amazon-ssm-agent from official AWS S3"
